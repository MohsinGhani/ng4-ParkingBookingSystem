...
export class SadarParkingComponent implements OnInit {
  currentUser;
  bookingData = {date:'' ,timeSlot:'',reserveHours:'',slotNumber:'',day:0 ,month:0,year:0,place:'CDGK'};
  dateAlert = false;
  timeAlert = false;
  selectTimeMode = true;
  ...
  selectTime(){
    if(this.dateValidation()){
        if(this.timeValidation()){
            this.slots = this._UserService.configurationOfCDGKslots(this.bookingData);
  ...
  agree(){
    if(this._UserService.reserveCDGKslot(this.bookingData)){
      this.selectTimeMode = false;
      this.selectSlotMode = false;
      this.agreeWithInfoMode = false;
