...
@Component({
  selector: 'app-gulshan-parking',
  templateUrl: './gulshan-parking.component.html',
  styleUrls: ['./gulshan-parking.component.css']
})
export class GulshanParkingComponent implements OnInit {

  bookingData = {date:'' ,timeSlot:'',reserveHours:'',slotNumber:'',day:0 ,month:0,year:0,place:'gulshan'};
  dateAlert = false;
  timeAlert = false;
  selectTimeMode = true;
  ...
  constructor(private _UserService:UserService) {
    _UserService.getGulshanReservedSlots();
  }
  ...
  selectTime(){
    if(this.dateValidation()){
        if(this.timeValidation()){
            this.slots = this._UserService.configurationOfGulshanSlots(this.bookingData);
  ...
  backToSelectTimeMode(){
    this._UserService.refreshGulshanSlots();
  ...
  agree(){
    if(this._UserService.reserveGulshanSlot(this.bookingData)){
      this.selectTimeMode = false;
      this.selectSlotMode = false;
      this.agreeWithInfoMode = false;
